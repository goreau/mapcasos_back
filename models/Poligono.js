...
class Poligono{
        ...
        try{
            ...
            switch (currUser.role) {
                case 1:
                    ...
                    break;
                case 2:
                    var terr = await this.getPoligono(currUser.id_municipio);
                    var id_regional =  terr.id_regional;

                    result = await knex.select(["id_municipio"])
                    .column(knex.raw("concat(to_char(id_municipio, '000'), ' - ', nome) as nome"))
                    .table("municipio as m")
                    .join("Poligono as t", "m.id_regional", '=' , "t.id_Poligono")
                    .where({'t.id_Poligono': id_regional})
    ...
    async getPoligono(id){
        try{
            var result =  await knex.select(["id_municipio", "t1.id_Poligono as id_regional", "t1.nome as regional", "t2.id_Poligono as id_drs", "t2.nome as drs", "t3.id_Poligono as id_colegiado", "t3.nome as colegiado"])
            .table("municipio as m")
            .join("Poligono as t1", "m.id_regional", '=', "t1.id_Poligono")
            .join("Poligono as t2", "m.id_drs", '=', "t2.id_Poligono") 
            .join("Poligono as t3", "m.id_colegiado", '=', "t3.id_Poligono")
    ...
    async getPoligonos(tipo, id){
        try{
            if (tipo == 9){
                var result =  await knex.select(["t.id_municipio as id", "t.nome"])
                .table("municipio as t");              
            } else {
                var result =  await knex.select(["t.id_Poligono as id", "t.nome"])
                .table("Poligono as t")
                .where('tipo', tipo); 
            }
